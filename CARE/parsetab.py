...
_lr_productions = [
  ...
  ('program -> add_conditions','program',1,'p_program','care_parse.py',16),
  ('program -> create_illness','program',1,'p_program','care_parse.py',17),
  ('program -> remove_illness','program',1,'p_program','care_parse.py',18),
  ('program -> remove_patient','program',1,'p_program','care_parse.py',19),
  ('program -> queue','program',1,'p_program','care_parse.py',20),
  ('program -> attend','program',1,'p_program','care_parse.py',21),
  ('create_illness -> AILMENT LP ID RP PERIOD CREATE LP RP','create_illness',8,'p_create_illness','care_parse.py',27),
  ('remove_illness -> AILMENT LP ID RP PERIOD REMOVE LP RP','remove_illness',8,'p_remove_illness','care_parse.py',34),
  ('function -> ID','function',1,'p_function','care_parse.py',42),
  ('create_patient -> ID PERIOD CREATE LP RP','create_patient',5,'p_create_patient','care_parse.py',53),
  ('add_conditions -> AILMENT LP ID RP PERIOD ADD LP ID RP','add_conditions',9,'p_add_conditions','care_parse.py',60),
  ('add_symptoms -> ID PERIOD HAS LP ID RP','add_symptoms',6,'p_add_symptoms','care_parse.py',67),
  ('list_symptoms -> ID PERIOD LIST LP RP','list_symptoms',5,'p_list_symptoms','care_parse.py',74),
  ('remove_patient -> ID PERIOD REMOVE LP RP','remove_patient',5,'p_remove_patient','care_parse.py',81),
  ('diagnose_patient -> ID PERIOD DIAGNOSE LP RP','diagnose_patient',5,'p_diagnose_patient','care_parse.py',88),
  ('specific_diagnose -> ID PERIOD AILMENT LP ID RP PERIOD DIAGNOSE LP RP','specific_diagnose',10,'p_specific_diagnose','care_parse.py',95),
  ('queue -> ID LP ID RP','queue',4,'p_queue','care_parse.py',102),
  ('attend -> ID LP RP','attend',3,'p_attend','care_parse.py',109),
]
